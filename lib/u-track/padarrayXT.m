...
padSize = varargin{2};
%iptcheckinput(padSize, {'double'}, {'real' 'vector' 'nonnan' 'nonnegative' 'integer'}, mfilename, 'PADSIZE', 2);
validateattributes(padSize, {'double'}, {'real' 'vector' 'nonnan' 'nonnegative' 'integer'}, mfilename, 'PADSIZE', 2);
...
if nargin > 2
    ...
    if ~ischar(varargin{3})
        ...
        padVal = varargin{3};
        %iptcheckinput(padVal, {'numeric' 'logical'}, {'scalar'}, mfilename, 'PADVAL', 3);
    ...
    for k = firstStringToProcess:nargin
        validStrings = {'circular' 'replicate' 'symmetric' 'antisymmetric' 'pre' ...
            'post' 'both'};
        %string = iptcheckstrs(varargin{k}, validStrings, mfilename, 'METHOD or DIRECTION', k);
        string = validatestring(varargin{k}, validStrings, mfilename, 'METHOD or DIRECTION', k);
...
function out = mkconstarray(class, value, sizeA)
...
out = repmat(feval(class, value), sizeA);
